...
waste, so make the question explicit so it can be checked later, whether the
...
One file, plain HTML/CSS/JS: no framework, no bundler, no server, no CDN
dependencies, everything inline so it opens by double-click, offline, and
...
2. **Current state**: the full relevant state, rendered as a readable panel
   ...
   changed.
3. **Free-play buttons**: one button per action, always available, so anyone can
   poke at the model in any order. Each click dispatches its action and
...
about on paper: the happy path, a tricky edge case, an attempt at something
that should be illegal.

Keep it beautiful but restrained: clean typography, generous spacing, one
accent colour. No animations, no gimmicks: nothing that competes with the state
and the buttons.

### 4. Hand it over

Send them the file, or open it for them. They'll click through the walkthroughs
and free-play whenever they get to it; the interesting moments are when they
say "wait, that shouldn't be possible" or "huh, I assumed X would be
