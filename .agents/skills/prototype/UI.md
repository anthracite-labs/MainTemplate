...
browser, picks one (or steals bits from each), then the rest are thrown away.
...
all stay. Only the rendering swaps. This is the default; pick it unless there's
...
- The project's component library / styling system (whatever the project
  already uses — TailwindCSS, shadcn, MUI, plain CSS). Use what is already in
  the project; add no CDN or new dependency for the prototype.
...
- **Variant label**: shows the current variant key and, if the variant exports a
  name, that name too. e.g. `B (Sidebar layout)`.
...
- Never shippable to production: the prototype lives on a branch or worktree
  that is reviewed before anything merges, and the switcher is removed with the
  rest of the prototype at cleanup (step 6).
...
### 6. Record the answer, then delete

Once a variant has won, follow rule 6 in [SKILL.md](SKILL.md):

- record the question, the winning variant, and why (the evidence and
...
- do not push the variant set to a permanent branch by default. Retain it only
  when the task contract explicitly selects the prototype as durable evidence.
